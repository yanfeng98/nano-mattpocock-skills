...
Every skill in `engineering/` or `productivity/` (the **promoted** buckets) must have a reference in the top-level `README.md`, which is the authority on what the promoted set contains. Skills in `misc/` and `in-progress/` must not appear there. `scripts/link-skills.sh` consumes a wider set: it links every skill outside `misc/`, so `in-progress/` is installed locally while staying out of the README.
