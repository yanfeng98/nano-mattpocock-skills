...
Act whenever a promoted skill is added, renamed, or has its behaviour changed: create or re-sync its docs page. A rename moves the file too (`docs/<bucket>/<old>.md` → `docs/<bucket>/<new>.md`), because the published URL tracks the name; a skill that moves between `engineering/` and `productivity/` moves its docs file to the matching folder. Skills in `misc/` and `in-progress/` get no page, because neither of those buckets is promoted. A skill moving *out* of one of them into `engineering/` or `productivity/` gains a page; one moving the other way loses it.
...
**A page carries no install commands.** A page that writes them out hands the reader a second copy of the install story to keep in sync, and the copy is the one that goes stale. Install wording is a property of this repo's `README.md` and of [the install block](./install-block.md), not of the page.
