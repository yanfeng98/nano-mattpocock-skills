...
简报写明 **agent 该做什么**，这在两种入口上都适用：对 issue，是从零把改动做出来；对 PR，是*在既有 diff 上*还剩什么要做：把它做完、补上缺口、回应评审意见。两边原则相同；下面的 PR 示例展示了其中的差别。

## 原则

### 耐用优先于精确

这条 issue 可能在 `ready-for-agent` 上待几天甚至几周。期间代码库会变。写简报时，要让它在文件被改名、移动或重构之后依然有用。
...
agent 需要知道什么时候算做完。每份 agent 简报都必须有具体、可验证的验收标准（acceptance criteria）。每条标准都应当可以独立验证。
...
**Current behavior:**
当一条技能描述超过 1024 个字符，它会正好在 1024 个字符处截断，
完全不管词边界。这会产出在词中间结束的描述
...
**Category:** enhancement
**Summary:** 把贡献者那份给 `triage list` 用的 `--json` 输出开关做完

**Current behavior:**
这个 PR 加了一个 `--json` 开关，把 issue 列表序列化成 JSON。正常
路径能跑通，diff 也符合项目的命令结构。还差两处：
错误仍然按人读的文本打印（而不是 JSON），而且这个新开关没有
测试覆盖。

**Desired behavior:**
带上 `--json` 时，所有输出（包括错误）都是 stdout 上格式良好的 JSON，
命令的退出码保持不变。不带这个开关时，现有的人读输出
原样不动。
